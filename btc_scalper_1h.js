'use strict';
/**
 * BTC Polymarket 1-HOUR Trend-Following Bot
 * ==========================================
 * Strategy : Entry $0.71 | TP $0.82 | SL $0.57
 * Logic    : Hourly trend-following — enters on momentum confirmation.
 * Mode     : Runs in DRY-RUN by default (no real orders).
 *            Pass --live flag to execute real trades.
 *
 * How it works:
 *   1. Discovers the current 1-hour BTC market on Polymarket.
 *   2. Watches for YES or NO price to reach $0.71 (trend confirmation).
 *   3. Enters the trade and monitors for TP ($0.82) or SL ($0.57).
 *   4. If neither triggers, exits 120 seconds before market close.
 *   5. Saves all trade history to trades_1h.json for dashboard display.
 */
...
const BUY_PRICE      = 0.71;   // Entry price
const SELL_PRICE     = 0.82;   // Take-profit price
const STOP_LOSS      = 0.57;   // Stop-loss price
...
const TRADES_FILE      = path.join(__dirname, 'trades_1h.json');
...
// ─── LOGGING ──────────────────────────────────────────────────────────────────
const LOG = path.join(__dirname, 'bot_1h.log');
...
function recordTrade({ slug, side, entry, exit, pnl, type, entryTime, exitTime }) {
    ...
    VS.activeTrade = null;
    VS.trades.push({
        id: `1H-${Date.now()}`, market: slug, side, entry, exit,
        size: ORDER_SIZE_USD,
...
async function monitorTrade(market, tradeState) {
    ...
    while (true) {
        ...
        // Timer exit: close position 120 seconds before market end
        if (now >= endTs - 120) {
...
async function runCycle(market) {
    const { slug, yesTokenId, noTokenId, endTs } = market;
    info(`CYCLE: ${slug} | Watching for $${BUY_PRICE} trend breakout...`);
...
// ─── MAIN LOOP ────────────────────────────────────────────────────────────────
async function main() {
    info(`BTC 1-HOUR TREND started | DRY_RUN=${DRY_RUN}`);
    ...
    while (true) {
        try {
            const now  = nowSec();
            const slug = getHourlySlug(now);

            if (slug === lastTradedSlug) { await sleep(30000); continue; }
